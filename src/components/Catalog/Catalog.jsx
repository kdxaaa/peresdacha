...
const Catalog = () => {
    ...
    return (
        <div className={style.catalog} id={'#catalog'}>
            ...
            <p className={style.select_p}>Выберите категорию</p>
            <select name="" id="" onChange={(e) => setCat(e.target.value)}>
                {
                    categories?.map((category) => {
                        return <option className="option1" value={category.name}>{category.name}</option>
                    })
                }
            </select>
            <div className={style.catalog_wrapper}>
                {
                    ...
                    <div className={style.tovar} key={item.id}>
                        ...
                        <div className={style.nam_tovar}>{item.name}</div>
